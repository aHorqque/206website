...
#returns false if member is not a friend; returns true if member is a friend
def isFriend(elementOfTopic):
	foundFriend = False #default: is not friend
	with open('../data/members.csv','r') as csvfile:
		reader = csv.reader(csvfile)
		for row in reader:
			if row[1] == currentusername: #finds row of current user
				for i, element in enumerate(row, start=3):
					if element == elementOfTopic:
						foundFriend = True #is friend
	return foundFriend

#search if element of row is member
#eturn false if it is a member; returns true if it is a member
def isMember(member):
	foundMember = False
	with open('../data/members.csv','r') as csvfile:
		reader = csv.reader(csvfile)
...
def printFeed():
	...
	if (os.stat('../data/topic.csv').st_size != 0): #if file is not empty
	...
	else: #if there is not post/file empty
		print "<br>"
...
def addFriend():
	if (postForm.getvalue("formname") == "addfriend"):
		if isMember(postForm.getvalue("friend")) == False:
			print "This user is not a member."
		else:
			new_rows_list = []
			with open('../data/members.csv', 'r') as csvfile:
				reader = csv.reader(csvfile)
				for row in reader:
					if row[1] == currentusername: #If it is on the current user's row
						foundFriend = False
